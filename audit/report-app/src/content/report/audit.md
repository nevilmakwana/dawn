# GREY EXIM â€” COMPLETE WEBSITE AUDIT
...
- Local JSON/JSONC files parsed successfully after honoring Shopifyâ€™s generated leading comments; no missing `{% render %}` or `{% section %}` targets were found.
...
1. **GE-001 â€” Confirmed â€” P0:** Return-policy URL linked from cart returns 404.
2. **GE-002 â€” Confirmed â€” P1:** GoAffPro loader is executed twice and throws on every sampled page.
3. **GE-003 â€” Confirmed â€” P1:** Collection/search mobile LCP exceeds 7 seconds in the lab profile.
4. **GE-004 â€” Confirmed â€” P1:** Mobile search input is 14 px, retaining iOS auto-zoom risk.
5. **GE-005 â€” Confirmed â€” P1:** Search sort select lacks an effective accessible name in Lighthouse.
...
- Three sticky collection rowsâ€”title, category tabs, and toolbarâ€”remain present after scroll on mobile, consuming a large part of a 320 px viewport.
...
Representative URL: [Womenâ€™s Scarves](https://www.greyexim.com/collections/women-scarves).
...
- Colour swatches render at about 13 Ã— 13 px and fail touch-target sizing.
...
- Cart page exposes three H1s: â€œShopping bag (0)â€, â€œYour cartâ€, and â€œYour shopping bag is emptyâ€. Use a single page H1 and downgrade state/subsection headings.
...
- Search pageâ€™s main input is 14 px; newsletter input is also 14 px. Both can trigger iOS zoom.
...
- Meta `fbevents.js` ~108 KB plus signals config ~83â€“84 KB.
...
- Lighthouse estimated only 5â€“27 KB image-delivery savings on sampled routes, so images are not the only bottleneck.
...
- Lighthouse found ~98â€“105 KB unused JS on every sampled route.
...
- Meta scripts transfer roughly 191â€“192 KB in sampled runs.
...
- Several text links have only 14â€“20 px height.
- Contact page lacks H1; cart heading hierarchy is ambiguous.

Fix targets: 24 Ã— 24 px minimum safe target (prefer 44 Ã— 44 px for primary touch controls), visible focus, meaningful labels, semantic table `<th scope>`, alt text, and logical headings.
...
9. Keep mobile hero below ~500â€“700 KB where visual quality allows.
...
Each row follows **WHAT â†’ WHERE â†’ WHY â†’ HOW â†’ EXPECTED RESULT â†’ HOW TO VERIFY**.
...
| GE-003 | P1 Confirmed | Collection LCP 7.7 s | Mobile PLP | Poor CWV/conversion | Prioritize first image; reduce blocking/global work | Lab LCP <2.5â€“3 s | 3-run median Lighthouse |
...
| GE-007 | P1 Confirmed | 13 px colour targets | Product tiles | Mistaps/accessibility | 24â€“44 px hit wrapper; keep 13 px dot | Reliable selection | Lighthouse target-size + touch |
...
| GE-012 | P1 Confirmed | Back-link label mismatch | Article subbar | Accessible name differs from â€œJOURNALâ€ | Align visible text and aria-label | Predictable announcement | Lighthouse label-name |
...
| GE-019 | P1 Confirmed | 195â€“297 requests | All pages | Latency/CPU overhead | Remove apps, combine ownership, lazy load | Smaller waterfalls | Request budget CI |
...
**Phase 0 â€” same day:** GE-001, GE-002, GE-005, GE-006, GE-010â€“012, GE-017. These are narrow, measurable fixes.

**Phase 1 â€” 2â€“4 days:** collection/search LCP, request/JS audit, touch targets, search toolbar, product contrast, heading hierarchy.

**Phase 2 â€” 1â€“2 weeks:** support and nested-collection navigation redesign; app/embed inventory; global checkout/analytics payload reduction; responsive regression suite.

**Phase 3 â€” ongoing:** CSS architecture cleanup, header modularization, product consolidation/redirect plan, RUM for image failures and CWV, real-device/browser matrix.
...
| Metric | Budget |
|---|---:|
| Mobile LCP lab median | â‰¤2.5 s target; â‰¤3.0 s interim |
| Field INP p75 | â‰¤200 ms |
| CLS | â‰¤0.10 |
| Mobile TBT lab | â‰¤200 ms |
| Initial transfer, non-home | â‰¤1.5 MB interim; â‰¤1.0 MB target |
| Initial transfer, media home | â‰¤2.0 MB interim |
| Requests | â‰¤150 interim; â‰¤100 target |
| First-party JS transfer | â‰¤250 KB |
| Third-party JS transfer | â‰¤150 KB before explicit user interaction/consent |
